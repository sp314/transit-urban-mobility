...
Queen <- as.nb.sgbp(st_queen(travel_time_sf %>% filter(year == 2017, month==1, week==3)))
```
...
dc_sar = spautolm(formula = mean_travel_time ~ as.numeric(distance), data = travel_time_sf %>% filter(year == 2017, month==1, week==3), listw = listW, family = "SAR")

summary(nc_sar)
```

```{r}
# dc$sar_resid <- resid(dc_sar)
# 
# travel_time_sf %>%
#   ggplot()+
