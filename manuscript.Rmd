...
travel_time_sf <- travel_time_sf %>%
  ...
  mutate(march_inaug = 0)

travel_time_sf[11122:11676, "march_inaug"] = 1
#entries 11122 to 11676
```
...
#Averaged travel times over weeks 3 and 4 in January
...
Queen <- as.nb.sgbp(st_queen(mod1data))
...
dc_sar = spautolm(formula = MeanTimeNorm ~ med_incomeE + med_incomeM + asianE + asianM + blackE + blackM + public_transitE + public_transitM + car_transitE + car_transitM + NumRoads + NumMetro+distance, data = mod1data, listw = listW, family = "SAR")
...
```{r CSR_test}
listW = nb2listw(Queen)
dc_sar2 = spautolm(formula = MeanTimeNorm ~ NumRoads+NumMetro+distance, data = mod1data, listw = listW, family = "SAR")
...
```{r CSR_test}
listW = nb2listw(Queen)
dc_sar2 = spautolm(formula = MeanTimeNorm ~ med_incomeE + asianE + blackE + public_transitE + NumRoads + NumMetro+distance, data = mod1data, listw = listW, family = "SAR")
...
```{r}
QueenDiff <- as.nb.sgbp(st_queen(mod3data))
W <- nb2mat(Queen, style='B', zero.policy = TRUE)
...
```{r}
listW = nb2listw(QueenDiff)
dc_sar_diff = spautolm(formula = MeanTimeNorm ~ med_incomeE + asianE + blackE + public_transitE + NumRoads + NumMetro+distance, data = mod3data, listw = listW, family = "SAR")
